...
def oneEpoch(e, dataset):
    ...
    setIter = dataset.batchs_argProb()
    for words, inMask, maskL, maskM, maskR, argRole, label in setIter:
        opt.zero_grad()
        words, inMask, maskL, maskM, maskR, argRole, label = words.to(device), inMask.to(device), maskL.to(device), maskM.to(device), maskR.to(device), argRole.to(device), label.to(device)
        loss, socres, preds = discriminator(words, inMask, maskL, maskM, maskR, argRole, label)
        loss.backward()
        opt.step()
        ...
        if cnt % 100 == 0:
            sys.stdout.write("%s step %d | loss: %f, acc_not_NA: %f, acc_NA: %f, acc_total: %f\r" % (time_str, cnt, loss, acc_not_NA.get(), acc_NA.get(), acc_total.get()))            sys.stdout.flush()
        cnt+=1
